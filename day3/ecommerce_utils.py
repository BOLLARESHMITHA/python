...
def apply_discount(price, discount_percent):
    dis_p=price-(price*discount_percent)/100
    return dis_p
...
def generate_invoice(cart, discount_percent=0, gst_percent=18):
    prices=list(cart.keys())
    Subtotal=0
    print("------ INVOICE ------")
    for i in prices:
        print(i,"\t: ",cart.get(i))
        Subtotal+=int(cart.get(i))
    print("---------------------\nSubtotal: ₹",Subtotal)
    d_price=round(apply_discount(Subtotal,discount_percent),1)
    print("After ",discount_percent,"% discount: ₹",d_price)
    print("After ",gst_percent,"% GST: ₹",round(add_gst(d_price),2))
    print("---------------------\nThank you for shopping with us!")
